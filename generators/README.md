...
### 2. Dev Python harness — `dev/run_harness.sh`

```bash
./generators/dev/run_harness.sh
```

Dispatches [dev/harness_relmotion.py](dev/harness_relmotion.py) into an already-running Rhino 8
...
`dev/inputs/*.json` (edit `INPUT_FILE` in the harness to switch cases). Bakes all intermediate
breps / edges / rails into the live Rhino doc for hand inspection, and drops per-step diagnostic
output into `dev/last_run_report.txt`.
