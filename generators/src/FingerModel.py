...
class FingerParams:
    ...
    # When True, ignores phalanx circumferences and uses simple tapered cylinders
    suppress_mid_phalanx_bulge: bool = False
...
def construct_next_joint_and_phalanx(
    ...
    joint_brep = create_sphere(new_line.From, sphere_radius, tolerance)
    ...
    if mid_radius is not None:
        phalanx_brep = create_bulged_cylinder(
            new_line, joint_begin_radius, mid_radius, joint_end_radius, tolerance
        )
    else:
...
def create_finger_model(
    ...
    # Convert phalanx mid-circumferences to radii (used for bulge if not suppressed)
    if params.suppress_mid_phalanx_bulge:
        proximal_mid_radius = None
        middle_mid_radius = None
        distal_mid_radius = None
    else:
        proximal_mid_radius = params.proximal_circ / (2 * math.pi) + shell
        middle_mid_radius = params.middle_circ / (2 * math.pi) + shell
        distal_mid_radius = params.distal_circ / (2 * math.pi) + shell
