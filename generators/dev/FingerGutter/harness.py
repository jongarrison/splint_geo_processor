"""Dev harness: run the FingerGutter pipeline against one or more dev inputs and bake the named
outputs for each into the live Rhino doc for side-by-side visual inspection.

WHY THIS EXISTS
    Fastest edit-run-eyeball cycle for algorithm work. Calls the same
    `FingerGutterGenerator.generate()` entrypoint production uses, so any change to the src modules
    is immediately exercised end-to-end without touching Grasshopper. All previewable geometry comes
    back in the `debug` dict - the harness bakes only what's named there.
    ...
    contains FingerGutter-specific logic: which inputs to run and which debug keys to bake.

LAYOUT
    Each input file gets its own row along +Y. Inside a row, phases step out along +X with
    auto-generated colors (golden-ratio HSV spacing). Set STOP_AFTER to a phase number to halt
    early and preview only completed phases.
...
SWITCHING INPUTS
    ...
    raw_data payload with the exact keys FingerGutterGenerator.generate() reads.
"""
...
# None = run full pipeline. Examples: 1.0 stops after the blank, 2.0 after the trim solid.
STOP_AFTER = None
...
_HERE = Path(__file__).resolve().parent               # .../generators/dev/FingerGutter
...
# Reload the module chain so edits since the Rhino session started are picked up. FingerGutter
# reloads its own geometry deps (FingerModel, BrepDifference) on import, so reloading it cascades;
# splintcommon is reloaded first since FingerGutter pulls symbols from it.
import FingerGutter
reload(FingerGutter)
from FingerGutter import FingerGutterGenerator
...
def main():
    report("=== FingerGutter dev harness ===")
    bk.clear_doc()
    ...
    for row_index, (name, path) in enumerate(input_paths):
        ...
        report("running FingerGutterGenerator.generate (objectID {0})...".format(object_id))

        generator = FingerGutterGenerator()
        debug = {}
        stopped_early = False
        try:
            generator.generate(raw_data, object_id, debug=debug, stop_after=STOP_AFTER)
        ...
        # Row label text-dot above the final solid (or blank, if stopped early).
        ref_brep = debug.get("splint_solid") or debug.get("splint_blank")
